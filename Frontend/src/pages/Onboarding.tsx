import { ArrowLeft, ArrowRight, PersonStandingIcon, ScaleIcon, Target } from "lucide-react"
        ...
        <div className="p-6 pt-12 onboarding-wrapper">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center shrink-0">
              <PersonStandingIcon className="w-5 h-5 text-white" />
            </div>
        ...
        <div className="flex-1 px-6 onboarding-wrapper">
          {step === 1 && (
            <div className="space-y-6">
              ...
              <Input label="Age" type="number" className="max-w-2xl" value={formData.age} onChange={(v) => updateField('age', v)} placeholder="Enter yout age" min={13} max={120} required />
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6  onboarding-wrapper">
              <div className="flex items-center gap-4 mb-8">
                <div className="size-12 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 flex items-center justify-center"><ScaleIcon className="size-6 text-emerald-600 dark:text-emerald-400" />
                </div>
                <div >
              ...
              <div className="flex flex-col gap-4 max-w-2xl" >
                ...
                <Input label="Weight (kg)" type="number" className="max-w-2xl" value={formData.weight} onChange={(v) => updateField('weight', v)} placeholder="Enter yout weight" min={20} max={300} required />
                <Input label="height (cm) Optional" type="number" className="max-w-2xl" value={formData.height} onChange={(v) => updateField('height', v)} placeholder="Enter yout height" min={100} max={250} required />
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="space-y-6  onboarding-wrapper">
              <div className="flex items-center gap-4 mb-8">
                <div className="size-12 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 flex items-center justify-center"><Target className="size-6 text-emerald-600 dark:text-emerald-400" />
                </div>
                <div >
              ...
              <div className="border-t border-slate-200 dark:border-slate-200 my-6 max-w-lg">
                {/* Daily targets*/}
                <div className="space-y-8  max-w-lg">
                  <h3 className="text-md font-medium text-slate-800 dark:text-white mb-4 mt-4">Daily  Targets</h3>
                  <div className="space-y-6">
                    <Slider label="Daily Calorie Intake" min={120} max={4000} value={formData.dailyCalorieIntake}
                      onChange={(v) => updateField('dailyCalorieIntake', v)} unit="kcal" infoText="The total calories you plan to consume each day" />
                    <Slider label="Daily Calorie Burn" min={100} max={2000} value={formData.dailyCalorieBurn}
                      onChange={(v) => updateField('dailyCalorieBurn', v)} unit="kcal" infoText="The total calories you aim through excersize and  to activity each day." />
                  </div>
                </div>
              </div>
            </div>
